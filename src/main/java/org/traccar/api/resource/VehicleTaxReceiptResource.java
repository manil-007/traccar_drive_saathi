...
@Path("vehicleTaxReceipt")
@Produces(MediaType.APPLICATION_JSON)
@Consumes({ MediaType.APPLICATION_JSON, MediaType.APPLICATION_FORM_URLENCODED })
public class VehicleTaxReceiptResource {

    @Inject
    private Config config;

    private static final Logger LOGGER = LoggerFactory.getLogger(VehicleTaxReceiptResource.class);

    private static final String APISETU_PATH = "certificate/v3/transport/vhtax";
    ...
    public Response verify(@jakarta.ws.rs.core.Context jakarta.servlet.http.HttpServletRequest servletRequest) {
        try {
            ...
            LOGGER.warn("VehicleTaxReceipt verify failed: {}", e.getMessage());
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
