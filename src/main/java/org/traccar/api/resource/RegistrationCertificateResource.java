...
public class RegistrationCertificateResource {

    @Inject
    private Config config;

    private static final Logger LOGGER = LoggerFactory.getLogger(RegistrationCertificateResource.class);

    // Sandbox endpoint for Registration of Vehicles
    private static final String APISETU_PATH = "certificate/v3/transport/rvcer";
    ...
    public Response verify(@jakarta.ws.rs.core.Context jakarta.servlet.http.HttpServletRequest servletRequest) {
        try {
            ...
            LOGGER.warn("RegistrationCertificate verify failed: {}", e.getMessage());
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
