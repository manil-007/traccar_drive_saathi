...
public class FitnessCertificateResource {

    @Inject
    private Config config;

    private static final Logger LOGGER = LoggerFactory.getLogger(FitnessCertificateResource.class);

    // Use sandbox endpoint for now
    private static final String APISETU_PATH = "certificate/v3/transport/fitcer";
    ...
    public Response verify(@jakarta.ws.rs.core.Context jakarta.servlet.http.HttpServletRequest servletRequest) {
        try {
            ...
            LOGGER.warn("FitnessCertificate verify failed: {}", e.getMessage());
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
